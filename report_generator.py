...
# ── ข้อมูลสำหรับ Chart.js (แปลงเป็น JSON เพื่อฝังใน JavaScript) ──────────────
chart_labels  = json.dumps([ป["ชื่อ"] for ป in โครงการ], ensure_ascii=False)
chart_data    = json.dumps([ป["คืบหน้า"] for ป in โครงการ])
chart_colors  = json.dumps([สีแถบ(ป["คืบหน้า"]) for ป in โครงการ])
chart_borders = json.dumps([สีสถานะ.get(ป["สถานะ"], "#6b7280") for ป in โครงการ])
...
<div style="max-width:1200px;margin:0 auto;padding:32px 24px">
  ...
  <!-- กราฟความคืบหน้า -->
  <div class="section">
    <h2 style="color:#1d4ed8">ความคืบหน้าแต่ละโครงการ</h2>
    <div style="position:relative;height:260px">
      <canvas id="progressChart"></canvas>
    </div>
    <div style="display:flex;gap:20px;justify-content:center;margin-top:16px;font-size:.8em;color:#64748b">
      <span><span style="display:inline-block;width:12px;height:12px;border-radius:2px;background:#10b981;margin-right:4px"></span>75–100% (ดี)</span>
      <span><span style="display:inline-block;width:12px;height:12px;border-radius:2px;background:#f59e0b;margin-right:4px"></span>40–74% (ปานกลาง)</span>
      <span><span style="display:inline-block;width:12px;height:12px;border-radius:2px;background:#ef4444;margin-right:4px"></span>0–39% (ต้องเร่ง)</span>
    </div>
  </div>
